...
class ArgActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.button.setOnClickListener{
            ...
            val receivedValue = intent.getDoubleExtra("keyArg",0.0)
            if (receivedValue != 0.0) {
                conversion.res = (monto*receivedValue)
                conversion.tipoConversion = "Pesos Argentinos"
                conversion.cambio = receivedValue
                binding.montoArg.text=(monto*receivedValue).toString()
                binding.montoBolArg.text=monto.toString()
                refDb.push()
